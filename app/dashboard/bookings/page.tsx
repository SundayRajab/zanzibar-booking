...
export default function MyBookings() {
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      // Due to RLS, this securely fetches exactly this user's bookings only.
      // Ignores error so UI doesn't crash if user hasn't synced the schema yet.
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center bg-white dark:bg-zinc-900/50 rounded-3xl p-16 border border-zinc-200 dark:border-zinc-800 shadow-sm">
          ...
          <h2 className="text-2xl font-semibold text-black dark:text-white mb-2">No past bookings found</h2>
          <p className="text-zinc-500 dark:text-zinc-400">You haven't made any bookings yet! Take a look at our premium listings.</p>
        </div>
      ) : (
            ...
            <div key={booking.id} className="bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 flex flex-col sm:flex-row gap-6 hover:shadow-lg transition-shadow">
               <div className="w-full sm:w-48 h-32 rounded-xl bg-zinc-100 dark:bg-zinc-800 overflow-hidden shrink-0">
                 {booking.listings?.images?.[0] ? (
                   <img src={booking.listings.images[0]} className="w-full h-full object-cover" />
                 ) : (
                   <img src={`https://images.unsplash.com/photo-1590523277543-a94d2e4eb00b?w=400&q=80&sig=${booking.listings?.id}`} className="w-full h-full object-cover" />
                 )}
               </div>
               <div className="flex-grow flex flex-col">
                 <div className="flex justify-between items-start">
                   ...
                   <span className="px-3 py-1 bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400 rounded-full text-xs font-bold uppercase tracking-wider">
                     {booking.status}
                   </span>
                 </div>
                 <div className="mt-auto grid grid-cols-2 gap-4 border-t border-zinc-100 dark:border-zinc-800 pt-4">
                   <div>
                     <p className="text-xs text-zinc-500 uppercase tracking-wide">Date</p>
                     <p className="text-sm font-medium text-black dark:text-white">{new Date(booking.booking_date).toLocaleDateString()}</p>
                   </div>
                   <div>
