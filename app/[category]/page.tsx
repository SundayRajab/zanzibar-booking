...
export default function CategoryPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  const categoryStr = unwrappedParams.category
  ...
  const fetchCategoryListings = async () => {
    ...
    const { data, error } = await supabase
      .from("listings")
      .select("*")
      .eq("category", categoryStr.toLowerCase())
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 border-b border-zinc-200 dark:border-zinc-800 pb-8">
          ...
          <p className="mt-4 text-zinc-600 dark:text-zinc-400 text-lg">
            Explore the finest selection of {categoryTitle.toLowerCase()} curated for your perfect getaway.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center bg-white dark:bg-zinc-900/50 rounded-2xl p-16 border border-zinc-200 dark:border-zinc-800">
            <h2 className="text-2xl font-semibold text-black dark:text-white mb-2">No {categoryTitle} Available</h2>
