    ...
    if (error) {
      if (error.code === 'PGRST303' || error.message?.includes('JWT expired')) {
        ...
        const { data: retryData } = await supabase
          .from("listings").select("*").ilike("category", "%car%")
          .order("created_at", { ascending: false })
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 border-b border-zinc-200 dark:border-zinc-800 pb-8">
          ...
          <p className="mt-4 text-zinc-600 dark:text-zinc-400 text-lg">
            Explore the finest selection of cars curated for your perfect getaway.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center bg-white dark:bg-zinc-900/50 rounded-2xl p-16 border border-zinc-200 dark:border-zinc-800">
            <h2 className="text-2xl font-semibold text-black dark:text-white mb-2">No Cars Available</h2>
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <div className="flex items-end justify-between mt-auto">
                    <div>
                      ...
                      <p className="font-extrabold text-2xl text-black dark:text-white mt-0.5">${listing.price}<span className="text-sm font-normal text-zinc-500 dark:text-zinc-400">/day</span></p>
                    </div>
                  </div>
                </div>
              </Link>
